...
EXECUTION_VERSION = "mlb_execution_v1_2"
QUALITY_WIDTH = 0.85


@dataclass(frozen=True)
class MLBExecutionV12Config:
    ...
    max_state_age_minutes: float = 24 * 60
    require_fresh_capital_state: bool = False
    require_fresh_risk_state: bool = False
...
def _base_predictions(predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"execution v1.2 predictions missing required columns: {sorted(missing)}")
    ...
    for col in ["pred_home_score", "pred_away_score", "pred_total"]:
        ...
        if out[col].isna().any():
            raise ValueError(f"execution v1.2 predictions have invalid {col}")
...
def _winner_candidates(predictions: pd.DataFrame, cfg: MLBExecutionV12Config) -> pd.DataFrame:
    out = predictions.copy()
    out["market"] = "WINNER"
...
def _ou_candidates(predictions: pd.DataFrame, cfg: MLBExecutionV12Config, *, synthetic: bool) -> pd.DataFrame:
    out = predictions.copy()
    out["market"] = "OU"
    out["execution_mode"] = "synthetic_ou" if synthetic else "market_ou"
    if synthetic:
        if "synthetic_total_line" not in out.columns:
            raise ValueError("synthetic O/U candidates require precomputed synthetic_total_line")
        ...
        out["total_line"] = pd.to_numeric(out["synthetic_total_line"], errors="coerce")
    else:
        if "market_total_line" not in out.columns and "total_line" in out.columns:
            out["market_total_line"] = pd.to_numeric(out["total_line"], errors="coerce")
...
def _apply_batch_quality(candidates: pd.DataFrame, cfg: MLBExecutionV12Config) -> pd.DataFrame:
    out = candidates.copy()
    for (mode, game_date), group in out[out["execution_action"].eq("ELIGIBLE")].groupby(["execution_mode", "game_date"], sort=False):
        ...
        avg_prob = ranked["heuristic_probability_score"].mean()
        avg_edge = ranked["edge_abs"].mean()
...
def _daily_cap(mode: str, slate_games: int, cfg: MLBExecutionV12Config) -> int:
...
def _stake_base(cfg: MLBExecutionV12Config, capital_state: dict | None) -> float:
...
def _state_age_minutes(state: dict | None) -> float | None:
    if not isinstance(state, dict) or "updated_at_utc" not in state:
        return None
    ts = pd.to_datetime(state.get("updated_at_utc"), utc=True, errors="coerce")
    if pd.isna(ts):
        return None
    return float((pd.Timestamp.now(tz="UTC") - ts).total_seconds() / 60.0)


def _state_reasons(cfg: MLBExecutionV12Config, capital_state: dict | None, risk_state: dict | None) -> list[str]:
    reasons: list[str] = []
    capital_age = _state_age_minutes(capital_state)
    risk_age = _state_age_minutes(risk_state)
    if cfg.require_fresh_capital_state and capital_state is None:
        reasons.append("MISSING_CAPITAL_STATE")
    if cfg.require_fresh_risk_state and risk_state is None:
        reasons.append("MISSING_RISK_STATE")
    if capital_state is not None and capital_age is None:
        reasons.append("INVALID_CAPITAL_UPDATED_AT")
    if risk_state is not None and risk_age is None:
        reasons.append("INVALID_RISK_UPDATED_AT")
    if capital_age is not None and capital_age > cfg.max_state_age_minutes:
        reasons.append("STALE_CAPITAL_STATE")
    if risk_age is not None and risk_age > cfg.max_state_age_minutes:
        reasons.append("STALE_RISK_STATE")
    return reasons


def _risk_multiplier(cfg: MLBExecutionV12Config, risk_state: dict | None, previous_action: str | None) -> tuple[float, list[str], bool]:
    multiplier = 1.0
    reasons: list[str] = []
...
def _select_orders(
    ...
    *,
    ...
    out["throttle_reason"] = ""
    out["final_order_rank"] = pd.NA
    multiplier, risk_reasons, kill = _risk_multiplier(cfg, risk_state, previous_action)
    state_reasons = _state_reasons(cfg, capital_state, risk_state)
    if state_reasons:
    ...
    out.loc[out["execution_action"].eq("THROTTLE"), "order_intent"] = "REDUCED_STAKE_ORDER_THROTTLE_IS_STILL_WAGER"
    ...
    out = _assign_final_order_rank(out)
    return out


def _assign_final_order_rank(out: pd.DataFrame) -> pd.DataFrame:
    result = out.copy()
    result["final_order_rank"] = pd.NA
    wagers = result[result["execution_action"].isin(["BET", "THROTTLE"])]
    for (_date, mode), group in wagers.groupby(["game_date", "execution_mode"], sort=True):
        ordered = group.sort_values(
            ["selection_score", "heuristic_probability_score", "edge_abs", "game_pk", "side"],
            ascending=[False, False, False, True, True],
            kind="mergesort",
        )
        result.loc[ordered.index, "final_order_rank"] = range(1, len(ordered) + 1)
    return result
...
def _american_profit(stake: pd.Series, price: pd.Series) -> pd.Series:
    odds = pd.to_numeric(price, errors="coerce")
    stake = pd.to_numeric(stake, errors="coerce").fillna(0.0)
    profit = pd.Series(0.0, index=stake.index, dtype=float)
    ...
    profit.loc[decimal] = stake.loc[decimal] * (odds.loc[decimal] - 1.0)
    profit.loc[american_positive] = stake.loc[american_positive] * odds.loc[american_positive] / 100.0
    profit.loc[american_negative] = stake.loc[american_negative] * 100.0 / odds.loc[american_negative].abs()
    return profit


def _price_for_wagers(wagers: pd.DataFrame) -> pd.Series:
    price = pd.Series(-110.0, index=wagers.index, dtype=float)
    if "market" not in wagers.columns:
        return price
    winner = wagers["market"].eq("WINNER")
    ou = wagers["market"].eq("OU")
    if "home_moneyline" in wagers.columns:
        price.loc[winner & wagers["side"].eq("HOME")] = pd.to_numeric(
            wagers.loc[winner & wagers["side"].eq("HOME"), "home_moneyline"],
            errors="coerce",
        ).fillna(price.loc[winner & wagers["side"].eq("HOME")])
    if "away_moneyline" in wagers.columns:
        price.loc[winner & wagers["side"].eq("AWAY")] = pd.to_numeric(
            wagers.loc[winner & wagers["side"].eq("AWAY"), "away_moneyline"],
            errors="coerce",
        ).fillna(price.loc[winner & wagers["side"].eq("AWAY")])
    if "total_price_over" in wagers.columns:
        price.loc[ou & wagers["side"].eq("OVER")] = pd.to_numeric(
            wagers.loc[ou & wagers["side"].eq("OVER"), "total_price_over"],
            errors="coerce",
        ).fillna(price.loc[ou & wagers["side"].eq("OVER")])
    if "total_price_under" in wagers.columns:
        price.loc[ou & wagers["side"].eq("UNDER")] = pd.to_numeric(
            wagers.loc[ou & wagers["side"].eq("UNDER"), "total_price_under"],
            errors="coerce",
        ).fillna(price.loc[ou & wagers["side"].eq("UNDER")])
    return price
...
def _summarize(frame: pd.DataFrame, period: Literal["overall", "daily", "weekly"]) -> pd.DataFrame:
    ...
    for keys, group in out.groupby(group_cols, sort=True):
        ...
        if "actual_result" in wagers:
            prices = _price_for_wagers(wagers)
            row["profit_units"] = float(
                (wagers["actual_result"].eq("WIN") * _american_profit(wagers["stake_final"], prices)).sum()
...
def execute_v1_2(
    ...
    *,
    config: MLBExecutionV12Config | None = None,
    capital_state: dict | None = None,
    risk_state: dict | None = None,
    previous_action: str | None = None,
) -> dict[str, pd.DataFrame]:
    cfg = config or MLBExecutionV12Config()
    ...
    if cfg.synthetic_ou_enabled:
        ...
        synth_input = synthetic_predictions.copy()
        if "total_line" in base.columns:
            synth_input["market_total_line"] = pd.to_numeric(base["total_line"], errors="coerce")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run governed MLB execution v1.2.")
    ...
    parser.add_argument("--winner-daily-cap", default=MLBExecutionV12Config.max_daily_winner_picks, type=int)
    parser.add_argument("--ou-daily-cap", default=MLBExecutionV12Config.max_daily_ou_picks, type=int)
    parser.add_argument("--slate-pick-fraction", default=MLBExecutionV12Config.slate_pick_fraction, type=float)
    parser.add_argument("--synthetic-policy", choices=["observe_only", "bet"], default=MLBExecutionV12Config.synthetic_policy)
    parser.add_argument("--require-market-validation-for-synthetic", action="store_true")
    parser.add_argument("--max-synthetic-market-gap", default=MLBExecutionV12Config.max_synthetic_market_gap, type=float)
    parser.add_argument("--max-game-exposure", default=MLBExecutionV12Config.max_game_exposure, type=float)
    parser.add_argument("--max-market-daily-exposure", default=MLBExecutionV12Config.max_market_daily_exposure, type=float)
    parser.add_argument("--max-total-daily-exposure", default=MLBExecutionV12Config.max_total_daily_exposure, type=float)
    parser.add_argument("--require-fresh-capital-state", action="store_true")
    parser.add_argument("--require-fresh-risk-state", action="store_true")
    parser.add_argument("--max-state-age-minutes", default=MLBExecutionV12Config.max_state_age_minutes, type=float)
...
def main() -> None:
    ...
    cfg = MLBExecutionV12Config(
        ...
        require_fresh_capital_state=args.require_fresh_capital_state,
        require_fresh_risk_state=args.require_fresh_risk_state,
        max_state_age_minutes=args.max_state_age_minutes,
    )
    outputs = execute_v1_2(
        predictions,
        config=cfg,
