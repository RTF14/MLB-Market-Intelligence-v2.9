...
from .ou_calibration_v1_7 import FEATURE_COLUMNS_CATEGORICAL_V17, FEATURE_COLUMNS_NUMERIC_V17
from .synthetic_totals import add_synthetic_total_line


EXECUTION_VERSION = "mlb_execution_v1_7"
EXPECTED_CALIBRATION_VERSION = "mlb_ou_calibration_v1_7"


@dataclass(frozen=True)
class MLBExecutionV17Config(MLBExecutionV12Config):
    ...
    max_vig: float = 0.08
    synthetic_divergence_penalty: float = 0.0
    odds_format: Literal["american", "decimal"] = "american"
    require_market_timestamps: bool = False
    max_market_data_age_minutes: float = 12 * 60
    min_batch_avg_probability: float = 0.0
    min_batch_avg_edge: float = 0.0
    slate_pick_fraction: float = 1.0
...
def _apply_winner_market_guards(predictions: pd.DataFrame, cfg: MLBExecutionV17Config) -> pd.DataFrame:
...
def _summarize_v1_7(frame: pd.DataFrame, period: Literal["overall", "daily", "weekly"], odds_format: Literal["american", "decimal"]) -> pd.DataFrame:
    if frame.empty:
        return pd.DataFrame()
    out = frame.copy()
...
def _calibrated_ou_candidates(predictions: pd.DataFrame, cfg: MLBExecutionV17Config, calibrator, metadata: dict) -> pd.DataFrame:
    ...
    out["total_line_open"] = _numeric_column(out, "total_line_open")
    out["total_line_move"] = _numeric_column(out, "total_line_move")
    missing_move = out["total_line_move"].isna() & out["total_line_open"].notna()
    out.loc[missing_move, "total_line_move"] = out.loc[missing_move, "market_total"] - out.loc[missing_move, "total_line_open"]
    out["total_price_over_open"] = _numeric_column(out, "total_price_over_open")
    out["total_price_under_open"] = _numeric_column(out, "total_price_under_open")
    out["price"] = np.where(
        out["side"].eq("OVER"),
        _numeric_column(out, "total_price_over"),
        _numeric_column(out, "total_price_under"),
    )
    out["selected_price"] = out["price"]
    out["selected_price_open"] = np.where(
        out["side"].eq("OVER"),
        out["total_price_over_open"],
        out["total_price_under_open"],
    )
    out["payout_per_unit"] = _profit_per_unit(pd.Series(out["price"], index=out.index), cfg.odds_format)
    out["implied_probability"] = _implied_probability(pd.Series(out["price"], index=out.index), cfg.odds_format)
    out["selected_implied_probability"] = out["implied_probability"]
    ...
    over_imp_open = _implied_probability(out["total_price_over_open"], cfg.odds_format)
    under_imp_open = _implied_probability(out["total_price_under_open"], cfg.odds_format)
    ...
    model_input = out[FEATURE_COLUMNS_NUMERIC_V17 + FEATURE_COLUMNS_CATEGORICAL_V17].copy()
    ...
    out["selection_score"] = (
        out["calibrated_ev"].fillna(-1) * 100.0
        + out["calibrated_probability"].fillna(0)
        + out["edge_abs"].fillna(0) / 100.0
    ).round(6)
...
def execute_v1_7(
    ...
    *,
    ...
    config: MLBExecutionV17Config | None = None,
    capital_state: dict | None = None,
    risk_state: dict | None = None,
    previous_action: str | None = None,
) -> dict[str, pd.DataFrame]:
    cfg = config or MLBExecutionV17Config()
    ...
    return {
        ...
        "daily_summary": _summarize_v1_7(selected, "daily", cfg.odds_format),
        "weekly_summary": _summarize_v1_7(selected, "weekly", cfg.odds_format),
        "overall_summary": _summarize_v1_7(selected, "overall", cfg.odds_format),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run MLB execution v1.7 with explicit price, metadata, and market freshness governance.")
    parser.add_argument("--predictions", required=True, type=Path)
    parser.add_argument("--ou-calibrator", required=True, type=Path)
    parser.add_argument("--out-dir", required=True, type=Path)
    parser.add_argument("--odds-format", choices=["american", "decimal"], default=MLBExecutionV17Config.odds_format)
